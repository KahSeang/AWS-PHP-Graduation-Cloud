    ...
    $stmt_events = $connection->prepare($sql_delete_events);
    $stmt_events->bind_param("i", $category_id);
    $stmt_events->execute();
    $stmt_events->close();

    $sql_delete_category = "DELETE FROM category WHERE category_id = ?";
    $stmt_category = $connection->prepare($sql_delete_category);
    $stmt_category->bind_param("i", $category_id);

    if ($stmt_category->execute()) {
        echo "<script>alert('Category and related events deleted successfully');</script>";
    } else {
